...
# - Manual robot drive control (WASD keys)
# - Autonomous navigation to ArUco markers
# - Manual arm control (Arrow keys + Q/E for gripper)
# 
# Navigation ESP32: 172.27.106.106:3333
# Arm ESP32: 172.27.106.192:3334
#
# Keyboard listener runs globally in background - no need to focus CV2 window!
# ============================================================================
...
JOINT_LIM = (20, 160)
GRIP_LIM = (0, 100)
HOME_BASE, HOME_JOINT, HOME_GRIP = 110, 160, 80
...
TARGET_DISTANCE_M = 0.4  # 30 cm in meters
LATERAL_THRESHOLD = 0.1  # 5 cm
DISTANCE_THRESHOLD = 0.1  # 5 cm

# PID Controller parametersqqqqqq
PID_LATERAL_KP = 0.15
...
# Global keyboard listener (always running)
...
def on_press(key):
    ...
    # Emergency stop - 'E' key immediately switches to manual control
    try:
        if hasattr(key, 'char') and key.char and key.char.lower() == 'e':
            if current_state == SystemState.NAVIGATING:
                # Emergency override - switch to manual navigation immediately
                current_state = SystemState.MANUAL_NAVIGATION
                manual_nav_running = True
                send_nav_command("S")  # Stop immediately
                logger.info("🚨 EMERGENCY STOP - Switching to manual control")
                print("\n" + "=" * 70)
                print("🚨 EMERGENCY MANUAL OVERRIDE ACTIVATED")
                print("=" * 70)
                print("You now have full manual control. Use WASD to drive.")
                print("Press ESC to exit manual mode.")
                print("=" * 70 + "\n")
                held.clear()
                held_special.clear()
                return
    ...
    if key == keyboard.Key.esc:
        if current_state == SystemState.ARM_CONTROL:
            ...
            return  # Don't return False - keep listener running!
    ...
    try:
        ...
        # Note: O/C for gripper control are handled in main loop when held
...
def main():
    ...
    global held, held_special, manual_nav_running
    ...
    print("\n⚠️  SAFETY FEATURES:")
    print("  - EMERGENCY STOP: Press 'E' at any time for instant manual control")
    print("  - MANUAL OVERRIDE: WASD keys work during autonomous navigation")
    ...
    print("  Arrow ← - rotate base right  |  Arrow → - rotate base left")
    print("  Arrow ↑ - joint up           |  Arrow ↓ - joint down")
    ...
    last_arm_keepalive = 0  # Track last arm keepalive time
    
    print(f"\n✅ System ready! Current state: {current_state}")
    print("Press 'm' (manual drive) | 'n' (autonomous nav) | 'a' (arm control)...\n")
    
    # Start global keyboard listener (runs in background)
    ...
    while True:
        # ARM KEEPALIVE: Send pose every 500ms when connected but not in ARM_CONTROL
        # This prevents ESP32 timeout failsafe from resetting to HOME
        if arm_sock and current_state != SystemState.ARM_CONTROL:
            current_time = time.time()
            if current_time - last_arm_keepalive > 0.5:  # 500ms = 2Hz keepalive
                send_arm_pose()
                last_arm_keepalive = current_time
        ...
        if current_state == SystemState.WAITING_TO_START:
            ...
            if key == ord('q'):
                ...
                # Send current arm pose (maintain existing position)
                send_arm_pose()
                ...
                print("  R                - return to home position (releases gripper!)")
                print("  P                - print pose")
                print("  ESC              - exit (maintains current position)")
                print("ℹ️  Arm maintains position when switching modes")
                print("=" * 70 + "\n")
                
                # Activate arm control mode
                ...
                try:
                    ...
                    # If arm has been moved from HOME, ensure connection is maintained
                    if not arm_sock and (base != HOME_BASE or joint != HOME_JOINT or grip != HOME_GRIP):
                        logger.info("Arm has been used - connecting to maintain position during navigation")
                        try:
                            endpoint = connect_arm()
                            print(f"✅ Connected to arm ESP32 to maintain position: {endpoint}")
                        except Exception as e:
                            logger.warning(f"Could not connect to arm ESP32: {e}")
                            print(f"⚠️  Warning: Arm ESP32 not connected - arm may reset to HOME")
        ...
        # ====================================================================
            ...
            cv2.putText(frame, "WASD/Arrows: Drive | QE: Diagonals | X: Stop | N: Auto Nav | ESC: Exit",
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
            
            cv2.imshow("Integrated Robot System", frame)
            
            # Process manual navigation commands (WASD + Arrow keys)
            cmd = None
            
            # WASD keys
            if 'w' in held or keyboard.Key.up in held_special:
                cmd = "F"  # Forward
            elif 's' in held or keyboard.Key.down in held_special:
                cmd = "B"  # Backward
            elif 'a' in held or keyboard.Key.left in held_special:
                cmd = "L"  # Left
            elif 'd' in held or keyboard.Key.right in held_special:
                cmd = "R"  # Right
            elif 'q' in held:
                cmd = "G"  # Forward-Left
            elif 'e' in held:
        ...
        # ====================================================================
            ...
            if ids is not None:
                ...
                # logger.debug(f"Detected markers: {detected_ids}")
                ...
                for i, corner in enumerate(corners):
                    ...
                    if success:
                        ...
                        if marker_id == ROBOT_ID:
                            ...
                            robot_rvec = rvec
                            # logger.debug(f"Robot marker (ID {ROBOT_ID}) detected: 2D center={center_2d}, 3D pos={position_3d}")
                        elif marker_id == destination_id:
                            dest_center_2d = center_2d
                            dest_pos_3d = position_3d
                            # logger.debug(f"Destination marker (ID {destination_id}) detected: 2D center={center_2d}, 3D pos={position_3d}")
                    else:
            ...
            # Manual override - check for WASD input (always active during navigation)
            manual_override_cmd = None
            if 'w' in held:
                manual_override_cmd = "F"  # Forward
            elif 's' in held:
                manual_override_cmd = "B"  # Backward
            elif 'a' in held:
                manual_override_cmd = "L"  # Left
            elif 'd' in held:
                manual_override_cmd = "R"  # Right
            elif 'q' in held:
                manual_override_cmd = "G"  # Forward-Left
            elif 'e' in held:
                manual_override_cmd = "I"  # Forward-Right
            elif 'z' in held:
                manual_override_cmd = "H"  # Backward-Left
            elif 'c' in held:
                manual_override_cmd = "J"  # Backward-Right
            elif 'x' in held:
                manual_override_cmd = "S"  # Stop
            
            # If manual override is active, use it instead of autonomous navigation
            if manual_override_cmd:
                send_nav_command(manual_override_cmd)
                status_text = f"🎮 MANUAL OVERRIDE: {manual_override_cmd} | Press 'E' to fully switch to manual mode"
            
            elif robot_center_2d is not None and dest_center_2d is not None:
                if robot_pos_3d is not None and dest_pos_3d is not None and robot_rvec is not None:
                    ...
                    if lateral_ok and distance_ok:
                        ...
                        print("  'a' - Manual arm control")
                        print("  'n' - Navigate to next destination")
                        print("  'm' - Manual drive mode")
                        print("  'q' - Quit system")
                        print("=" * 70 + "\n")
                    else:
                        ...
                        print(f"DEBUG: RobotFrame(x={x_robot:.3f}, y={y_robot:.3f}, z={z_robot_val:.3f}) "
                              f"dist={distance:.3f}m, dist_err={distance_error*100:.1f}cm, "
                              f"lat_err={lateral_error*100:.1f}cm, cmd={cmd}, phase={phase[:30] if phase else 'N/A'}")
                        ...
                        if cmd != last_cmd:
                            if time_since_last_change >= MIN_COMMAND_INTERVAL:
                                ...
                                # logger.debug(f"Command change rate limited: '{last_cmd}' -> '{cmd}' (only {time_since_last_change:.3f}s since last change, need {MIN_COMMAND_INTERVAL:.3f}s)")
                    ...
                    cmd = "S"
                    logger.warning(f"Missing 3D pose data: robot_pos_3d={robot_pos_3d is not None}, dest_pos_3d={dest_pos_3d is not None}, robot_rvec={robot_rvec is not None}")
                    pid_lateral.reset()
                    pid_distance.reset()
                    if cmd != last_cmd:
                        send_nav_command(cmd)
            ...
            elif not manual_override_cmd and robot_center_2d is None:
            ...
            elif not manual_override_cmd and dest_center_2d is None:
            ...
            cv2.putText(frame, "🚨 EMERGENCY: Press 'E' | OVERRIDE: Use WASD | STOP: Press 'S'",
                       (10, 85), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 165, 255), 1)
            
            # Display robot frame information if both markers detected
            if robot_center_2d is not None and dest_center_2d is not None:
                y_offset = 90
                cv2.putText(frame, f"Robot (ID {ROBOT_ID}): ({robot_center_2d[0]}, {robot_center_2d[1]}) px",
                           (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(frame, f"Dest (ID {destination_id}): ({dest_center_2d[0]}, {dest_center_2d[1]}) px",
                           (10, y_offset + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
                
                # Display robot frame information if available
                if robot_pos_3d is not None and dest_pos_3d is not None and robot_rvec is not None:
                    dest_in_robot_frame_vis, _ = transform_to_robot_frame(
                        robot_pos_3d, robot_rvec, dest_pos_3d
                    )
                    lateral_error_vis, distance_error_vis, distance_vis, angle_deg_vis = compute_robot_frame_errors(
                        dest_in_robot_frame_vis, TARGET_DISTANCE_M
                    )
                    
                    x_robot_vis, y_robot_vis, z_robot_vis = dest_in_robot_frame_vis
                    distance_cm_vis = distance_vis * 100
                    
                    cv2.putText(frame, f"Robot Frame: x={x_robot_vis:.3f}m y={y_robot_vis:.3f}m z={z_robot_vis:.3f}m",
                               (10, y_offset + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
                    cv2.putText(frame, f"Distance: {distance_cm_vis:.1f} cm | Lateral err: {lateral_error_vis*100:.1f} cm | Angle: {angle_deg_vis:.1f}°",
        ...
        # ====================================================================
            ...
            cv2.putText(frame, "Press 'a' (arm) | 'n' (next dest) | 'm' (drive) | 'q' (quit)",
            ...
            if key == ord('q'):
                ...
                print("\n" + "=" * 70)
                dest_input = input("Enter next destination marker ID: ")
                try:
                    destination_id = int(dest_input)
                    ...
                    pid_distance.reset()
                    last_cmd = None
                    logger.info(f"Starting navigation to next marker ID {destination_id}")
                    print(f"✅ Navigation started to marker ID {destination_id}")
                    print("=" * 70 + "\n")
                except ValueError:
                    print("❌ Invalid ID. Please try again.")
                ...
                # Start manual navigation mode
                current_state = SystemState.MANUAL_NAVIGATION
                logger.info("Starting manual navigation mode from NAVIGATION_COMPLETE")
                print("\n" + "=" * 70)
                print("🎮 MANUAL DRIVE MODE ACTIVATED")
                print("=" * 70)
                print("\nMANUAL DRIVE CONTROLS:")
        ...
        # ====================================================================
            ...
            # Process arm movements (identical to manual_arm_control_nowifi.py)
            moved = False
            
            # Arrow keys (left/right swapped for intuitive control)
            if keyboard.Key.left in held_special:  base += STEP_DEG; moved = True  # Swapped: left arrow -> rotate right
            if keyboard.Key.right in held_special: base -= STEP_DEG; moved = True  # Swapped: right arrow -> rotate left
            ...
            # Gripper (O/C instead of Q/E to avoid quit conflict)
            if 'o' in held: grip -= STEP_GRIP; moved = True
            if 'c' in held: grip += STEP_GRIP; moved = True
            
            # CRITICAL: Always send pose to prevent ESP32 WiFi timeout failsafe
            # ESP32 has 800ms timeout - if no commands received, it returns to HOME
            # Unlike serial version, WiFi TCP requires continuous keepalive at 30Hz
            send_arm_pose()
            
            # Use time.sleep for consistent timing
            time.sleep(1.0 / ARM_SEND_HZ)
            
            # Check if user wants to exit arm control
            if not arm_control_running:
                logger.info("Exiting arm control mode (keeping connection alive)")
                # DON'T disconnect - keep arm connection alive to maintain position
                # If we disconnect, ESP32 failsafe returns arm to HOME
            ...
            # Check for quit command via cv2 window (non-blocking)
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                logger.info("Quit key pressed")
                arm_control_running = False
